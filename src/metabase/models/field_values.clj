...
(def advanced-field-values-max-age
  "Age of a advanced field-values in days.
...
(def ^:private advanced-field-values-types
  ...
  TODO: some note about advanced field values"
  #{:sandbox         ;; are fieldvalues but has constraints from other locked parameters on a dashboard/embedding
    :locked-filter}) ;; are fieldvalues but are filtered by sandbox permissions

(def ^:private field-values-types
  "A full list of all possible field values type"
...
(defn- pre-update [{:keys [id type field_id] :as field-values}]
  (u/prog1 field-values
    (assert-valid-human-readable-values field-values)
    (assert-valid-field-values-type field-values)
    (when (= (or type (db/select-one-field :type FieldValues :id id))
             :full)
...
(defn advanced-fieldvalues-expired?
  ...
  (t/before? (:created_at fv)
         (t/minus (t/offset-date-time)
                  (t/days advanced-field-values-max-age))))
